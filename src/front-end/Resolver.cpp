...
Object Resolver::visitVariableExpr(shared_ptr<Variable<Object>> expr) {
    if (scopes.size() != 0) {
        ...
        if (searched != last.end() && !searched->second) {
            Error::addError(expr->name, "Can't read local "
...
Object Resolver::visitSelfExpr(shared_ptr<Self<Object>> expr) {
    if (currentClass == CLASS_NONE) {
        Error::addError(expr->keyword, "Cannot use 'self' outside of a class.");
        return Object::make_none_obj();
    }
...
void Resolver::visitClassStmt(const Class &stmt) {
    ...
    if (stmt.superclass != nullptr &&
        ...
        Error::addError(stmt.superclass->name, "A class cannot inherit from itself.");
    }
...
void Resolver::visitReturnStmt(const Return &stmt) {
    if (currentFunction == FUNCTION_NONE) {
        Error::addError(stmt.name, "Cannot return from top-level code.");
    }
    ...
    if (stmt.value != nullptr) {
        ...
        //     Error::addWarn(
        //         stmt.name,
        //         "Warn! Return int type in initializer by default."
        //     );
        // }
        resolve(stmt.value);
    }
...
void Resolver::declare(Token name) {
    ...
    if (scope.find(name.lexeme) != scope.end()) {
        Error::addError(name, "Variable with the name '" + name.lexeme + "' already exists in this scope");
    }
